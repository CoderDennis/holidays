defmodule Holidays.Definitions.De do
  ...
  def init() do
    ...
    holiday(
      "Karfreitag",
      %{regions: [:de], function: {Holidays, :easter, [:year], -2}}
    )

    holiday(
      "Ostermontag",
      %{regions: [:de], function: {Holidays, :easter, [:year], 1}}
    )

    holiday(
      "Christi Himmelfahrt",
      %{regions: [:de], function: {Holidays, :easter, [:year], 39}}
    )

    holiday(
      "Pfingstmontag",
      %{regions: [:de], function: {Holidays, :easter, [:year], 50}}
    )
    ...
    holiday(
      "Neujahrstag",
      %{month: 1, regions: [:de], day: 1}
    )

    holiday(
      "Heilige Drei Könige",
      %{month: 1, regions: [:de_bw, :de_by, :de_st], day: 6}
    )
    ...
    holiday(
      "Mariä Himmelfahrt",
      %{month: 8, regions: [:de_by, :de_sl], day: 15}
    )

    holiday(
      "Friedensfest",
      %{month: 8, regions: [:de_by_aux], day: 8}
    )
    ...
    holiday(
      ...
      %{month: 10, regions: [:de], day: 3}
    )

    holiday(
      "Reformationstag",
      %{month: 10, regions: [:de_bb, :de_mv, :de_sn, :de_st, :de_th], day: 31}
    )
    ...
    holiday(
      "Allerheiligen",
      %{month: 11, regions: [:de_bw, :de_by, :de_nw, :de_rp, :de_sl], day: 1}
    )
    ...
    holiday(
      "1. Weihnachtstag",
      %{month: 12, regions: [:de], day: 25}
    )

    holiday(
      "2. Weihnachtstag",
      %{month: 12, regions: [:de], day: 26}
    )
  end
